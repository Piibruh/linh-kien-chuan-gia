...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setErrors({ email: typeof data?.error === 'string' ? data.error : 'Đăng ký thất bại' });
        setIsLoading(false);
        return;
      }

      if (data?.token && data?.user) {
        useAuthStore.setState({ user: data.user, token: data.token, isLoggedIn: true });
        setSuccess(true);
      ...
      setErrors({ general: 'Không kết nối được máy chủ. Hãy chạy npm run dev:all hoặc dev:server.' });
      setIsLoading(false);
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        {/* Logo */}
        <Link to="/" className="flex items-center justify-center gap-3 mb-8 text-left">
          <BrandMark size="lg" />
